    ...
    <div class="mb-4">
        <div class="flex items-center w-full" v-show="state.edit === false">
            <input type="checkbox" class="mr-2"			 	   
				   ...
				   @click="updateTodo">
            <p class="w-auto"
				      :class="data.completed ? 'line-through text-green' : 'text-grey-darkest cursor-pointer hover:text-black hover:font-bold'"
					  @click="startEdit">
					  {{todo.title}}
			</p>
        ...
        <div class="flex items-center w-full" v-show="state.edit === true">
            <input class="appearance-none border rounded w-full py-2 px-3 mr-2 text-black"
				   v-model="data.title"
				   @keyup.enter="updateTodo"
				   placeholder="Update Todo">
            <button class="flex-no-shrink p-2 ml-2 border-2 rounded text-teal border-teal
					hover:text-white hover:bg-teal"
					@click="updateTodo"
					:disabled="data.title.length === 0">Update
			</button>
...
<script>
  export default{
    ...
    mounted() {
      ...
      t.data.completed = t.todo.completed;
    },
    methods: {
      updateTodo() {
        const t = this;
